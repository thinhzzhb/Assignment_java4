            ...
            <div class="card">
                ...
                <div class="card-body">
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        <c:forEach var="items" items="${video}">
                        <tr>
                            <td><a href="<c:url value='/video?action=watch&id=${items.videoid}'/>">${items.videoid}</a></td>
                            <td>${items.title}</td>
                            <td>${items.views}</td>
                            <td>${items.active?"Đang phát hành":"Ngừng phát hành"}</td>
                            <td>
                                <a type="button" href="<c:url value="/admin/video?action=edit&id=${items.videoid}"/>" class="btn btn-block btn-info btn-sm" >Edit</a>
                                <button type="button" onclick="deleteVideo('${items.videoid}')" class="btn btn-block btn-danger btn-sm" >Delete</button>
                            </td>
                        </tr>
                        </c:forEach>
                        </tbody>
...
<script>
    function deleteVideo(videoid) {
        $.ajax({
            url: '/Assignment_java4_thinhlhph19459/admin/video?action=delete&id='+videoid
        }).then(function (data) {
            alert("Successfully Deleted!")
            window.location.href = "http://localhost:8080/Assignment_java4_thinhlhph19459/admin/video?action=view";
        }).fail(function (error) {
